export const Chart = (() => {
	...
	const init = {
		...
		globalStyle: {
			...
			xSplitAxisColor: 'rgb(220, 225, 229)',
			xSplitAxisDotted: false,

			ySplitAxisColor: 'rgb(220, 225, 229)',
			ySplitAxisDotted: false,
			...
			orderBookBuyItemColor: 'rgb(0,168,0)',
			orderBookSellItemColor: 'rgb(252,4,4)'
		},

		layerType: 'candle', // candle , line
		layerStyle: {
			candle: {
				incrementItemColor: '#28a745',
				decrementItemColor: '#dc3545',
	...
	const themes = {
		...
		dark: {
			globalStyle: {
				...
				orderBookBuyItemColor: '#14b143',
				orderBookSellItemColor: '#ef232a'
			},
			layerStyle: {
				candle: {
	...
	const overwrite = (target, base) => {
		...
		for (let key in base) {
			if (key.substr(0, 1) === '$' || target[key] === undefined) target[key] = base[key];
		}
	...
	class Chart {
		constructor(domId) {
			...
			let initialTooltipFormatter = df;
			const setInitialTooltipFormatter = formatter => {
				initialTooltipFormatter = formatter;
			};
			...
			const yLabelCtx = makeCanvas(1).context;
			const xLabelCtx = makeCanvas().context;

			const supportCtx = makeCanvas(1).context;
			const floatCtx = makeCanvas(2).context;
			const tooltipCtx = makeCanvas(3).context;
			...
			const getItemColorForTypes = (layer, index) => {
				if (layer.type === 'candle') {
					...
					if (close > open) return layer.style.incrementItemColor;
					if (close < open) return layer.style.decrementItemColor;
					return globalStyle.labelColor;
			...
			const renderYLabel = () => {
				...
				for (let i = -10; i <= split + 10; i++) {
					...
					if (yLabelShow !== false) {
						let text = yLabelFormatter(d);

						yLabelCtx.fillStyle = globalStyle.backgroundColor;
						yLabelCtx.fillRect(yLineX + (yLabelAlign === 'right' ? 10 : -10), y - 6, (yLabelAlign === 'right' ? 1 : -1) * yLabelCtx.measureText(text).width, 12);

						yLabelCtx.fillStyle = globalStyle.labelColor;
						yLabelCtx.strokeStyle = globalStyle.labelColor;

						yLabelCtx.fillText(text, yLineX + (yLabelAlign === 'right' ? 10 : -10), y);
			...
			const renderForTypes = {
				candle: (ctx, {
					...
					if (close > open) ctx.fillStyle = ctx.strokeStyle = incrementItemColor;
					else if (close < open) ctx.fillStyle = ctx.strokeStyle = decrementItemColor;
					else ctx.fillStyle = ctx.strokeStyle = globalStyle.labelColor;
			...
			const renderLastIndex = () => {
				supportCtx.save();
				supportCtx.clearRect(-10, -10, wrapper.clientWidth + 20, wrapper.clientHeight + 20);
				for (let layerName in layers) {
					let layer = layers[layerName];
					if (layer.style.renderLastIndex === false || layer.show === false) continue;

					let lastIndex = viewport[1] - 1,
						realPrice;
					if (layer.type === 'line' && layer.style.horizon === true) lastIndex = 0;

					if (layer.data[lastIndex] === undefined) continue;
					...
					_focusIndex({
						...
						index: viewport[1] - 1,
						realPrice,
						focusYLabelColor: layer.style.lastIndexColor,
				...
				(() => {
							...
							if (notRender !== true) {
								...
								if (Array.isArray(text.text) === true) {
									...
									for (let j = 0; j < text.text.length; j++) {
										ctx.fillText(text.text[j], lineWidth + l + j * space, 0);
					...
					return (index) => {
						...
						if (dateFormatter !== undefined) {
							...
							texts.push({
								text,
								color: globalStyle.tooltipTextColor
							});
						}
						...
						for (let name in layers) {
							...
							if (filters.indexOf(name) !== -1 || layer.show === false) continue;

							let text = tooltipForTypes[layer.type]
								({
									title: titles[name] || name,
									formatter: (formatters && formatters[name]) || initialTooltipFormatter,
			...
			const setOrderBook = p => {
				orderBook = overwrite(p, orderBook);
				renderOrderBook();
			};

			const renderOrderBook = () => {
				let ctx = orderBook.$dom.context;
				let {
					tWidth,
					tHeight
				} = getTransformSize();
				...
				ctx.save();
				ctx.clearRect(-10, -10, wrapper.clientWidth + 20, wrapper.clientHeight + 20);
				ctx.translate(grid.left + (globalStyle.yLabelAlign === 'left' ? -grid.left : tWidth), 0);
				ctx.translate(2, 0);

				let a = (globalStyle.yLabelAlign === 'left' ? globalStyle.yLabelWidth : 0),
					b = (globalStyle.yLabelAlign === 'left' ? 0 : globalStyle.yLabelWidth);

				/* buy order */
				ctx.strokeStyle = globalStyle.orderBookBuyItemColor;
				ctx.beginPath();
				let fy, px, py;

				for (let i in orderBook.buy) {
					let price = orderBook.getPrice(orderBook.buy[i]),
						total = orderBook.getTotal(orderBook.buy[i]);

					let x = map(total, 0, orderBook.maxTotal, a, b),
						y = map(price, max, min, grid.top + padding.top, grid.top + tHeight - padding.bottom);
				...
				let grd = ctx.createLinearGradient(0, 0, globalStyle.yLabelWidth, 0);
				grd.addColorStop(0, globalStyle.orderBookBuyItemColor);
                ...
                /* sell order */
				ctx.strokeStyle = globalStyle.orderBookSellItemColor;
				ctx.beginPath();
				px = undefined, py = undefined, fy = undefined;

				for (let i in orderBook.sell) {
					let price = orderBook.getPrice(orderBook.sell[i]),
						total = orderBook.getTotal(orderBook.sell[i]);
				...
				grd = ctx.createLinearGradient(0, 0, globalStyle.yLabelWidth, 0);
				grd.addColorStop(0, globalStyle.orderBookSellItemColor);
			...
			const resize = () => {
				...
				if (previousWrapperSize.width === wrapper.clientWidth &&
					previousWrapperSize.height === wrapper.clientHeight) return;
				...
				previousWrapperSize.width = wrapper.clientWidth;
				previousWrapperSize.height = wrapper.clientHeight;
			};
			...
			$method.refreshTimeline = () => {
				$timeline = $methodByKey[$key].getTimeline();
				renderAll();
			};
			...
			this.setOrderBook = makePublic(setOrderBook);
			this.setInitialTooltipFormatter = makePublic(setInitialTooltipFormatter)

			this.getLayerStyle = layerName => layers[layerName].style;
